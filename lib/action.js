...
action.unchecked = function unchecked(action_path) {
  ...
  if (typeof parsedAction === 'string') {
    ...
    handler = function(connection) {
      ...
      connection.controller.mode = _.underscored(parsedAction.name);
      actionHelpers.bind_route_parameters(connection.controller.constructor.prototype[parsedAction.name],
...
action.accessChecked = function accessChecked(action_path) {
  ...
  return function(connection) {
    AccessControl.checkAccess(connection.route, connection, function(err, allow) {
      if (allow) {
        handler(connection);
      }
      else {
