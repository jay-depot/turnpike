...
function EndpointController(connection) {
  ...
  this.deliver['text/html'] = function() {
    var page = new application.controllers.Page(connection);;
    var view = this.renderable();
...
/**
 * Makes this constructor the prototype of {ctor} and attaches any
 * static methods on this constructor, including this one, to
 * {ctor}.
 *
 * Use this in lieu of util.inherits when defining controllers.
 *
 * @param {function} ctor
 */
EndpointController.extend = function(ctor) {
  var i;

  require('util').inherits(ctor, this);
  for (i in this) {
    //noinspection JSUnfilteredForInLoop
    ctor[i] = this[i];
  }
